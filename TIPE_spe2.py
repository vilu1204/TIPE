    ...
    def move(self, Dx, Dy):
        if self.alive :
            ...
            if Dx != 0.0 and Dy != 0.0:
                x0 = (self.x + self.speed* Dx) % self.batch_ref.dim
                y0 = (self.y + self.speed* Dy) % self.batch_ref.dim
                self.x = x0
                self.y = y0
                self.eat_around()
    ...
    def gather_input_data(self):
        ...
        for f in self.batch_ref.list_fishes :
            if f.species != self.species:
                res.append(((f.x-x0+d)%dist-d)/dist) 
                res.append(((f.y-y0+d)%dist-d)/dist)
...
b1 = batch([10,10,10],[1,1,1],[20,20,20],[1,1,1],50, [[False,False,False],[True,False,False],[False,True,False]], [61, 11, 2], 50, [2,2,2])
b1.create()
...
# b1.list_fishes[0].session(prey1,pred1,500)
# b1.list_fishes[10].session(prey1,pred1,500)
# b1.list_fishes[20].session(prey1,pred1,500)
...
pickle.dump((b1) ,open('E:charsave_v2_base_up', 'wb'))

##

# b1 = pickle.load(open('/media/alefebvre/3713-4F08/charsave', 'rb'))
b1 = pickle.load(open('E:charsave2098', 'rb'))
